...
class Intellivue():
    def __init__(self, ttyDEV, ttyBaud, mac):
        ...
        self.MDSExtendedPollActionNumeric = self.decoder.writeData('MDSExtendedPollActionNUMERIC', dataCollection)
    ...
    def fields_counter(self, res):
        ...
        print(f'{time.time() - self.initiation_time:0.2f}', self.successes, self.total, ",".join([str(self.fields[k]) for k in self.fields]))
        ...
        try:
            ...
            if message_type == 'AssociationResponse':
                ...
                try:
                    ...
                    self.manufacturer="".join([str(f"{i:#04x}")[2:4] for i in self.last[0]["MDSCreateInfo"]["MDSAttributeList"]["AttributeList"]["AVAType"]["NOM_ATTR_ID_MODEL"]["AttributeValue"]["SystemModel"]["manufacturer"]["VariableLabel"]["value"]])
                    self.DID="".join([str(f"{i:#04x}")[2:4] for i in self.last[0]["MDSCreateInfo"]["MDSAttributeList"]["AttributeList"]["AVAType"]["NOM_ATTR_SYS_ID"]["AttributeValue"]["VariableLabel"]["value"]])
                    self.vent_type="".join([str(f"{i:#04x}")[2:4] for i in self.last[0]["MDSCreateInfo"]["MDSAttributeList"]["AttributeList"]["AVAType"]["NOM_ATTR_ID_MODEL"]["AttributeValue"]["SystemModel"]["model_number"]["VariableLabel"]["value"]])
                    self.status=self.last[0]["MDSCreateInfo"]["MDSAttributeList"]["AttributeList"]["AVAType"]["NOM_ATTR_MODE_OP"]["AttributeValue"]["OperatingMode"]
                except:
                    pass
                logging.info(message_type)
    ...
    def close(self):
        ...
        while True:
            ...
            if message_type == None:
                ...
                break
        logging.critical(f"Closing, total uptime was {time.time() - self.initiation_time:0.2f}")
        self.status="DISCONNECT"
    ...
    def poll(self):
        try:
            ...
            if message_type in ["LinkedMDSExtendedPollActionResult"|"MDSExtendedPollActionResult"]:
