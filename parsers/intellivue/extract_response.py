...
def getdata(d, acc):
    ...
    if RemoteOperationType=="ROLRS_APDU":#first and second, with seconds using RorlsId state of RORLS_LAST
        print("Recieved ROLRS")
        RemoteOperationLinkedResult=d.get("ROLRSapdu",{"RolrsId":{"state":"NONE","Rolrs_count":0}})
    ...
    singlecontexts=[pollinfolist[f"SingleContextPoll_{x}"] for x in range(int(pollinfolist["count"]))]
    #print(f"\n\n{singlecontexts=}")
    for singlecontext in singlecontexts:
        pollinfo=singlecontext["SingleContextPoll"]["poll_info"]
        observations=[pollinfo[f"ObservationPoll_{x}"] for x in range(int(pollinfo["count"]))]
        #print(observations)
        for observation in observations:
            ...
            for attribute in attributelist.keys():
                ...
                if attribute=="NOM_ATTR_NU_VAL_OBS":
                    print("name=",attributelist[attribute]["AttributeValue"]["NuObsValue"]["SCADAType"])
                    print("data=",attributelist[attribute]["AttributeValue"]["NuObsValue"]["FLOATType"])
                    rolslist+=[attributelist[attribute]["AttributeValue"]["NuObsValue"]]
                if attribute=="NOM_ATTR_NU_CMPD_VAL_OBS":
                    compoundvalues=attributelist[attribute]["AttributeValue"]["NuObsValCmp"]
                    values=[compoundvalues[f"NuObsValue_{x}"] for x in range(int(compoundvalues["count"]))]
                    #print(f"{values=}")
                    for val in values:
