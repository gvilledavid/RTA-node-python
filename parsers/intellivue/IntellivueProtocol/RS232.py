...
class RS232(object):
    ...
    def __init__(self, specifiedPort, bdrate):
        ...
        logging.debug('Trying to open serial connection to device at {0}:{1}'.format(specifiedPort, bdrate))

        self.socket = serial.Serial(port=specifiedPort,
                                    baudrate=bdrate, bytesize=serial.EIGHTBITS,
    ...
    def receive(self):
        ...
        # If not at start bit return nothing
            ...
            # logging.warning(f"not at start bit {currentByte}")
            # messageByte = self.socket.read(1000)
            # message = bytearray()
            # while True:
            #     messageByte = self.socket.read(1)
            #     # message = message + messageByte
            #     # logging.warning(messageByte)
            #     if not messageByte or messageByte == b'\xC1': break
            finalMessage = b''
    ...
    def send(self, message):
        ...
        msg = self.frameCheckWrite(message)
...
if __name__ == '__main__':
    ...
    ser = RS232('/dev/ttyUSB0', 19200) # or 115200
