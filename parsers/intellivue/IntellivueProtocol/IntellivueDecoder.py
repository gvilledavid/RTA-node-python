...
class IntellivueDecoder(object):
    ...
    def __init__(self):
        ...
        self.DataKeys["PollProfileExtOptions"] = {
            ...
            "POLL12SECANDWAVEANDLISTANDDYN": b"\x4f\x00\x00\x00",
            ...
            b"\x4f\x00\x00\x00": "POLL12SECANDWAVEANDLISTANDDYN",
    ...
    def loadPhysioLabels(self):
        ...
        with open(
            os.path.join(package_directory, "PhysioLabels.txt"), "r"
        ) as label_file:
            labels = []
            descriptions = []
            numbers = []
    ...
    def readData(self, data):
        ...
        # Output parameters necessary for MDSCreateEventResult
            ...
            logging.debug(
                "MDSCreateEvent->message_dict: {0}".format(current_message_dict)
            )
        ...
        for index, tuples in enumerate(all_lengths_sorted):
            ...
            if len(tuples) == 3:
                if length <= 127:
                    ...
                    for add_tuples in all_lengths_sorted[
                        index + 1 : len(all_lengths_sorted)
                    ]:
    ...
    def recurseWrite(
        ...
        for data_type in message_list:
            ...
            if type(data_type) != list:
                # print("d: {0}".format(data_type))
                # print("p: {0}".format(parameters))
                # print("tp: {0}".format(type(parameters)))
                # print("p[d]: {0}".format(parameters.get(data_type)))
...
if __name__ == "__main__":
    from intelliview_raw import packets

    h = packets[0]
    fh = bytes.fromhex(h)

    ID = IntellivueDecoder()
    d = ID.readData(h)
    print(d)
